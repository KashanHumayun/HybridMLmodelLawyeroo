...
def update_experience(ref_path):
    ...
    for key, data in records.items():
        years = str(random.randint(1, 40))  # Generate random value between 1 and 40
        updates[f'{key}/years_of_experience'] = str(years)

    if updates:
        ref.update(updates)
        logger.info(f"Updated years of experience in {ref_path}")
    else:
        logger.info(f"All lawyers in {ref_path} have years of experience")


def main():
    update_experience('lawyers')
