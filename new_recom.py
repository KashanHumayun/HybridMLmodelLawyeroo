...
def retrain_collborative_model():
...
retrain_collborative_model()
try:
    test_client_id = "-NwBB-yOIGQlqLonV6Cx"  # Replace with an actual client ID
    if test_client_id is not None:
        recommended_lawyers = recommend_lawyers_using_collaborative_filtering(test_client_id, k=7)
        print("Recommended Lawyers:\n", recommended_lawyers)
    else:
        print("Client ID is None, cannot recommend lawyers.")
...
def get_recommendations():
    ...
    try:
        ...
        recomendationsforhybrid = recommend_lawyers_using_collaborative_filtering(client_id)
        recommended_lawyers = pd.concat([recommendations, recomendationsforhybrid])
