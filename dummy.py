...
def retrain_model():
    ...
    # Update the TensorFlow datasets
    global tf_client_features, tf_lawyer_features
    tf_client_features = tf.data.Dataset.from_tensor_slices(combined_client_features).batch(10)
    tf_lawyer_features = tf.data.Dataset.from_tensor_slices(specializations_binarized).batch(10)
    
    # Retrain the model
    hyb_model.fit(tf.data.Dataset.zip((tf_client_features, tf_lawyer_features)), epochs=10)
    hyb_model.save('hyb_model')  # Save the retrained model
    
    logger.info("Model retrained and saved successfully.")

# Set up a Firebase listener on the clients path
def on_client_change(event):
    # Log the event type and data for debugging
    logger.info(f"Firebase event triggered: {event.event_type} at path: {event.path}")
    logger.info(f"Data changed: {event.data}")
    
    # Call the retrain model function
    retrain_model()

# Attach the listener to the clients reference
db.reference('/clients').listen(on_client_change)
...
def trigger_retrain():
    try:
        retrain_model()
        return jsonify({'status': 'Model retraining initiated successfully'}), 200
